...
def add_rotation_augmentation_to_features_and_matches(
    image_list,
    images,
    features_path,
    matches_path,
        ...
        for angle in range(angle_step, 360, angle_step):
            ...
            if save_matches:
                for mimage in completed_images:
                    ...
                    write_matches(
                        matches_path, aug_name, mimage, matches_arr, scores_arr
                    )
                ...
                write_matches(
                    matches_path, image_name, aug_name, matches_arr, scores_arr
                )
...
def augment_images_and_points3D(outputs, image_dict):
    ...
    for point3D_id in points3d_bin:
        ...
        if point3D_id in points_3d:
            ...
            point2D_idxs = np.hstack(
                (point2D_idxs, points_3d[point3D_id]["point2D_idxs"])
            )
        aug_pt = Point3D(
            id=point3D_id,
            xyz=xyz,
