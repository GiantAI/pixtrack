...
def parse_args():
    ...
    parser.add_argument("--bin", default="bin", help="input path to the colmap bin files (set automatically if run_colmap is used)")
...
def run_colmap(args):
    ...
    if (input(f"warning! folders '{sparse}' and '{bin}' will be deleted/replaced. continue? (Y/n)").lower().strip()+"y")[:1] != "y":
        sys.exit(1)
    ...
    do_system(f"colmap model_converter --input_path {sparse}/0 --output_path {bin} --output_type TXT")
