...
def get_nerf_image(testbed, nerf_pose, camera, depth=False, alpha_thresh=0.0):
    spp = 8
    ...
    if depth:
        ...
        nerf_img = testbed.render(width, height, spp, True)
        nerf_img[nerf_img[:, :, 3] < alpha_thresh] = 0.0
...
def add_pose_axes(
    ...
    result_img = draw_axes(image, pts_3d, K, t=2.0)
    return result_img
...
def draw_points_with_lines(image, pts_3d, K=np.eye(3), t=15, color=(0, 50, 200)):
    ...
    while pt_num < len(pts_2d):
        ...
        image = cv2.line(
            image, pts_2d[pt_num], pts_2d[pt_num + 1], color=color, thickness=t
        )
        pt_num += 2
    return image
...
def add_object_center(image, camera, pose, object_center=None):
    ...
    object_center0 = np.hstack((object_center0, np.ones((object_center0.shape[0], 1))))
...
if __name__ == "__main__":
    ...
    for name_q in tqdm.tqdm(pose_stream):
        ...
        if "T_refined" in pose_stream[name_q]:
            ...
            nerf_img = get_nerf_image(testbed, nerf_pose, camera, alpha_thresh=0.0)
        else:
        ...
        if args.reference_image:
            result_img = add_reference_images(
                result_img, recon, ref_ids, sfm_images_dir
            )
        ...
        if not args.no_axes:
            result_img = add_pose_axes(
                result_img, camera, cIw_sfm, object_center.tolist() + [0]
            )
        ...
        if args.pose_error and "T_refined" in pose_stream[name_q]:
            ...
            rotation_error = geodesic_distance_for_rotations(gt_R, pr_R) * 180.0 / np.pi
            translation_error = np.linalg.norm(gt_T - pr_T) * 100.0
