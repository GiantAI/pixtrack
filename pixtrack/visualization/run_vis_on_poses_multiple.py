...
if __name__ == "__main__":
    ...
    testbed = initialize_ingp(str(nerf_path), background=[0.0, 0.0, 0.0, 0])
    ...
    for name_q in tqdm.tqdm(pose_stream):
        ...
        mask = np.array(
            (img != np.array([0, 255, 75])) * np.array([0, 255, 75]), dtype=np.uint8
        )

        # Mask clear is a segmentation mask in white, which can be used to mask out the object for multi-object tracking.
        mask_clear = np.array(
            (img != np.array([0, 255, 75])) * np.array([255, 255, 255]), dtype=np.uint8
        )
        ...
        result_img = cv2.drawContours(
            base_result_image.copy(), contours, -1, (0, 255, 75), 2
        )
        result_name = "result_%s" % os.path.basename(path_q)
        ...
        result_name = "result_%s" % os.path.basename(path_q)
        result_path_segmask = os.path.join(
            os.path.join(args.out_dir, "segmask"), result_name
        )
        ...
        result_name = "result_%s" % os.path.basename(path_q)
        result_path_segmask = os.path.join(
            os.path.join(args.out_dir, "cleared"), result_name
        )
